...
namespace BP.En
{
    ...
    abstract public class Entity : EnObj
    {
        ...
        protected virtual void afterUpdate()
        {
            ...
            if (this.EnMap.IsEnableVer == false)
                return;
            ...
            if (ver.RetrieveFromDBSources() == 0)
            {
                ...
                foreach (Attr attr in attrs)
                {
                    ...
                    dtl.MyPK = ver.No + "_" + attr.Key + "_" + dtl.EnVer;
                    dtl.Insert();
                }
                return;
            }
            ...
            //取出来版本数据.
            BP.Sys.EnVerDtls mydtls = new EnVerDtls();
            mydtls.Retrieve(EnVerDtlAttr.EnName, enName, EnVerDtlAttr.EnVer, "1");

            //检查是否有变更数据项目.
            BP.Sys.EnVerDtls dtlsChange = new EnVerDtls();
            foreach (EnVerDtl dtl in mydtls)
            {
                if (dtl.NewVal == this.GetValStrByKey(dtl.AttrKey))
                    continue;

                //设置翻转.
                dtl.OldVal = dtl.NewVal;

                //记录到最新的值到.
                dtl.NewVal = this.GetValStrByKey(dtl.AttrKey);

                //把变更的项目加入里面. 
                dtlsChange.AddEntity(dtl);
            }

            //如果没有变化.
            if (dtlsChange.Count == 0)
                return;

            foreach (EnVerDtl dtl in dtlsChange)
            {
                //更新原始版本数据.
                dtl.DirectUpdate();
                ...
                //把变化的数据插入.
                dtl.RDT = rdt;
                dtl.Rec = BP.Web.WebUser.No;
                dtl.EnVer = rdt;
                dtl.MyPK = ver.No + "_" + dtl.AttrKey + "_" + dtl.EnVer;
                dtl.Save();
            }
        ...
        private void CheckPhysicsTable_MySQL()
        {
            ...
            foreach (Attr attr in this._enMap.Attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppRate:
                    case DataType.AppDouble:
                        DBAccess.RunSQL("ALTER TABLE " + this._enMap.PhysicsTable + " ADD " + attr.Field + " FLOAT DEFAULT '" + attr.DefaultVal + "' NULL COMMENT '" + attr.Desc + "'");
                        break;
                    default:
                        throw new Exception("error MyFieldType= " + attr.MyFieldType + " key=" + attr.Key);
                }
