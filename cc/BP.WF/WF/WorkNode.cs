    ...
    public class WorkNode
    {
        ...
        private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, Int64 fid)
        {
            ...
            else
            {
                ...
                //增加天数. 考虑到了节假日.
                dtOfShould = DataType.AddDaysBak(DateTime.Now, this.town.HisNode.TSpanDay);

                // 增加小时数. 考虑到了节假日.
                dtOfShould = Glo.AddMinutes(dtOfShould, this.town.HisNode.TSpanMinues);
            }
            ...
            else
            {
                //计算警告日期。
                //增加天数. 考虑到了节假日.
                dtOfWarning = DataType.AddDaysBak(DateTime.Now, this.town.HisNode.TSpanDay);
                // 增加小时数. 考虑到了节假日.
                dtOfWarning = Glo.AddMinutes(dtOfWarning, (int)this.town.HisNode.TSpanHour*60);
            }
            ...
            try
            {
                ...
                if (this.HisNode.SFActiveFlows.Length > 2)
                {
                    ...
                    foreach (string flowNo in fls)
                    {
                        ...
                        //启动子流程.
                        BP.WF.Dev2Interface.Node_StartWork(fl.No, this.HisWork.Row, null, 0, null, this.WorkID, this.HisFlow.No);
                        msg += "@子流程[" + fl.Name + "]已经启动。";
                    }
                    if (msg != "")
