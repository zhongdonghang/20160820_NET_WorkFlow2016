...
namespace BP.WF
{
    ...
    public class Flow : BP.En.EntityNoName
    {
        ...
        public string DTSDBSrc
        {
            get
            {
                return this.GetValStringByKey(FlowAttr.DTSDBSrc);
            }
            set
            {
...
#warning 不应该出现的工作丢失.
        ...
        public string DoBTableDTS(GERpt rpt, Node currNode, bool isStopFlow)
        {
            ...
            string[] lcZDArray = dtsArray[0].Split(',');//取出对应的主表字段
            string[] lcZDType = dtsArray[1].Split(',');//主表字段类型
            string[] ywZDArray = dtsArray[2].Split(',');//取出对应的业务表字段

            //业务主表默认为nd**rpt，为空表明采取了是默认的表名
            string tableName = "ND" + int.Parse(this.No) + "Rpt";

            if (!string.IsNullOrEmpty(this.PTable))
                tableName = this.PTable.ToUpper();

            string sql = "SELECT " + dtsArray[0] + " FROM " + tableName + " WHERE OID=" + rpt.OID;
            DataTable lcDt = DBAccess.RunSQLReturnTable(sql);
            if (lcDt.Rows.Count == 0)//没有记录就return掉
                return "";

            sql = "SELECT " + dtsArray[2] + " FROM " + this.DTSBTable.ToUpper();
            DataTable ywDt = DBAccess.RunSQLReturnTable(sql);

            string values = "";

            BP.Sys.SFDBSrc sf = new BP.Sys.SFDBSrc();

            for (int i = 0; i < lcZDArray.Length; i++)
            {
                switch (sf.DBSrcType)
                {
                    ...
                    case Sys.DBSrcType.Oracle:
                        if (ywDt.Columns[ywZDArray[i]].DataType == typeof(DateTime))
                        {
                            if (!string.IsNullOrEmpty(lcDt.Rows[0][lcZDArray[i].ToString()].ToString()))
                            {
                                values += "to_date('" + lcDt.Rows[0][lcZDArray[i].ToString()] + "','YYYY-MM-DD'),";
                            }
                            else
                            {
                                values += "'',";
                        ...
                        values += "'" + lcDt.Rows[0][lcZDArray[i].ToString()] + "',";
                        continue;
                    default:
            ...
            sql = "INSERT INTO " + this.DTSBTable + "(" + dtsArray[2] + ") VALUES(" + values + ")";
            try
            {
                sf.RunSQL(sql);
            }
        ...
        public void DoDelete()
        {
            ...
            //删除视图.
            if (BP.DA.DBAccess.IsExitsObject("V_" + this.No))
                BP.DA.DBAccess.RunSQL("DROP VIEW V_" + this.No);

            //删除轨迹.
            if (BP.DA.DBAccess.IsExitsObject("ND" + int.Parse(this.No) + "Track")==true)
