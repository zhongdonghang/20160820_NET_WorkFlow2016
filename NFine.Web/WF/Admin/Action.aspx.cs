        ...
        public new string ShowType
        {
            get
            {
                if (this.NodeID == "0" && string.IsNullOrEmpty(this.FK_MapData)==false )
                    return "Frm";
                if (this.NodeID == "0" && this.FK_Flow.Length >= 3)
                    return "Flow";
                return "Node";
            }
        }
            ...
            foreach (EventSource item in ess)
            {
                ...
                foreach (BP.WF.XML.EventList xml in xmls)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Pub1.AddLi(string.Format("<div><a href='Action.aspx?NodeID={0}&Event={1}&FK_Flow={2}&MyPK={3}&tk={6}&FK_MapData={6}'><span class='nav'><img src='/WF/Img/Event.png' border=0/>" + msg + "{4}</span></a></div>{5}", NodeID, xml.No, FK_Flow, nde.MyPK, xml.Name, Environment.NewLine, new Random().NextDouble(), this.FK_MapData));
                        }
                    }
                }
