        ...
        public void BindCCForm(Entity en, MapData md, MapAttrs mattrs, string enName, bool isReadonly, float srcWidth, bool IsEnableLoadData)
        {
            ...
            foreach (FrmImg img in imgs)
            {
                ...
                if (img.IsEdit == 1 && this.IsReadonly == false)
                {
                    ...
                    queryInfo.DoQuery();
                    //判断是否存在
                    if (imgDb != null && !string.IsNullOrEmpty(imgDb.FK_MapData) & !IsPostBack) //edited by liuxc,2015-10-16,因新昌方面出现此处回发时将正确的签章图片路径改成了错误的路径，所以将此处的逻辑在回发时去掉，即回发时不进行签章图片的更新
                    {
                        ...
                        if (attr.UIContralType == UIContralType.DDL)
                        {
                            ...
                            else
                            {
                                ...
                                if (fSize == 0)
                                    ...
                                    tb.Attributes["style"] = "width: " + fSize + "px; text-align: left; height: " + fSize + "px;";
                        ...
                        switch (attr.MyDataType)
                        {
                            case BP.DA.DataType.AppString:
                                if (attr.UIRows == 1)
                                {
                                    ...
                                    if (fSize == 0)
                                        ...
                                        tb.Attributes["style"] = "font-size: " + fSize + "px;width: " + attr.UIWidth + "px; text-align: left; height: " + fSize + "px;padding: 0px;margin: 0px;";
                                    this.Add(tb);
                                }
                                else
                                {
                                    ...
                                    if (fSize == 0)
                                        ...
                                        tb.Attributes["style"] = "font-size: " + fSize + "px;width: " + attr.UIWidth + "px;height:" + attr.UIHeightInt + "px; text-align: left;padding: 0px;margin: 0px;height: " + fSize + "px;";
                            ...
                            case BP.DA.DataType.AppFloat:
                                ...
                                if (fSize == 0)
                                    ...
                                    tb.Attributes["style"] = "width: " + fSize + "px; text-align: right; height: " + fSize + "px;";
            ...
            foreach (MapDtl dtl in dtls)
            {
                ...
                {
                    if (isReadonly == true)
                        src = appPath + "WF/CCForm/Dtl.aspx?EnsName=" + dtl.No + "&RefPKVal=" + en.PKVal + "&IsReadonly=1&FID=" + en.GetValStrByKey("FID", "0") + "&FK_Node=" + this.Request.QueryString["FK_Node"].ToString();
                    else
                        src = appPath + "WF/CCForm/Dtl.aspx?EnsName=" + dtl.No + "&RefPKVal=" + en.PKVal + "&IsReadonly=0&FID=" + en.GetValStrByKey("FID", "0") + "&FK_Node=" + this.Request.QueryString["FK_Node"].ToString();
                }
                else
                {
                    if (isReadonly == true)
