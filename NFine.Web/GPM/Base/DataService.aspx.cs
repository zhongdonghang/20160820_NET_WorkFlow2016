    ...
    public partial class DataService : WebPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (method)
            {
                ...
                case "getstationbyname":// 岗位 模糊查找
                    ...
                    break;
                case "getflowtree"://获取流程类别和流程
                    s_responsetext = GetFlowTree();
                    break;
                case "getformtree"://获取表单库
                    s_responsetext = GetFormTree();
                    break;
                case "saveflowformmenu"://保存流程/表单菜单
                ...
                if (flowSort != null)
                {
                    ...
                    if (dt_sort != null && dt_sort.Rows.Count > 0)
                    {
                        foreach (DataRow row in dt_sort.Rows)
                        {
                            ...
                            flowTree.Append(",\"children\":");
                            flowTree.Append("[{");
                            flowTree.Append(string.Format("\"id\":\"{0}\",\"text\":\"{1}\"", row["No"].ToString() + "01", "加载中..."));
                            flowTree.Append("}]");
                            flowTree.Append("},");
                        }
                ...
                if (formSort != null)
                {
                    ...
                    if (dt_sort != null && dt_sort.Rows.Count > 0)
                    {
                        foreach (DataRow row in dt_sort.Rows)
                        {
                            ...
                            formTree.Append(",\"children\":");
                            formTree.Append("[{");
                            formTree.Append(string.Format("\"id\":\"{0}\",\"text\":\"{1}\"", row["No"].ToString() + "01", "加载中..."));
                            formTree.Append("}]");
                            formTree.Append("},");
                        }
        ...
        private void CreateFlowMenu(string FK_Flow, string curMenuNo)
        {
            ...
            if (flow != null && flow.Name != "")
            {
                ...
                flowMenu.Name = "发起" + flow.Name;
                flowMenu.MenuType = 4;
                flowMenu.Flag = "Start" + FK_Flow;
                flowMenu.Url = BP.WF.Glo.CCFlowAppPath + "WF/MyFlow.aspx?FK_Flow=" + FK_Flow + "&FK_Node=" + Convert.ToInt32(FK_Flow) + "01";
                flowMenu.WebPath = "/WF/Img/Start.gif";
                flowMenu.Update();
                //新增查询菜单
                flowMenu = menu.DoCreateSubNode() as BP.GPM.Menu;
                flowMenu.FK_App = menu.FK_App;
                flowMenu.Name = "查询" + flow.Name;
                flowMenu.MenuType = 4;
                flowMenu.Flag = "Search" + FK_Flow;
                flowMenu.Url = BP.WF.Glo.CCFlowAppPath + "WF/Rpt/Search.aspx?RptNo=ND" + Convert.ToInt32(FK_Flow) + "MyRpt&FK_Flow=" + FK_Flow ;
                flowMenu.WebPath = "/WF/Img/Search.gif";
                flowMenu.Update();
                //新增分析菜单
                flowMenu = menu.DoCreateSubNode() as BP.GPM.Menu;
                flowMenu.FK_App = menu.FK_App;
                flowMenu.Name = "统计" + flow.Name;
                flowMenu.MenuType = 4;
                flowMenu.Flag = "Group" + FK_Flow;
                flowMenu.Url = BP.WF.Glo.CCFlowAppPath + "WF/Rpt/Group.aspx?RptNo=ND" + Convert.ToInt32(FK_Flow) + "MyRpt&FK_Flow=" + FK_Flow;
                flowMenu.WebPath = "/WF/Img/Group.gif";
                flowMenu.Update();
            }
        }
        ...
        private void CreateFormMenu(string FK_MapData, string curMenuNo)
        {
            ...
            if (formMap != null && formMap.Name != "")
            {
                ...
                formMenu.Name = "新建"; // +formMap.Name;
                formMenu.MenuType = 4;
                formMenu.Flag = formMap.No;
                formMenu.Url = BP.WF.Glo.CCFlowAppPath + "WF/CCForm/Frm.aspx?FK_MapData=" + FK_MapData;
                formMenu.WebPath = BP.WF.Glo.CCFlowAppPath + "WF/Img/New.png";
                formMenu.Update();

                // 列表
                formMenu = menu.DoCreateSubNode() as BP.GPM.Menu;
                formMenu.FK_App = menu.FK_App;
                formMenu.Name = "列表"; //+formMap.Name;
                formMenu.MenuType = 4;
                formMenu.Flag = formMap.No;
                formMenu.Url = BP.WF.Glo.CCFlowAppPath + "WF/Rpt/SearchEUI.aspx?EnsName=" + FK_MapData;
                formMenu.WebPath = BP.WF.Glo.CCFlowAppPath + "WF/Img/Table.gif";
                formMenu.Update();

                // 查询
                formMenu = menu.DoCreateSubNode() as BP.GPM.Menu;
                formMenu.FK_App = menu.FK_App;
                formMenu.Name = "查询";// +formMap.Name;
                formMenu.MenuType = 4;
                formMenu.Flag = formMap.No;
                formMenu.Url = BP.WF.Glo.CCFlowAppPath + "WF/Comm/Search.aspx?EnsName=" + FK_MapData;
                formMenu.WebPath = BP.WF.Glo.CCFlowAppPath + "WF/Img/Search.gif";
                formMenu.Update();

                // 分析
                formMenu = menu.DoCreateSubNode() as BP.GPM.Menu;
                formMenu.FK_App = menu.FK_App;
                formMenu.Name = "统计"; // +formMap.Name;
                formMenu.MenuType = 4;
                formMenu.Flag = formMap.No;
                formMenu.Url = BP.WF.Glo.CCFlowAppPath + "WF/Comm/Group.aspx?EnsName=" + FK_MapData;
                formMenu.WebPath = BP.WF.Glo.CCFlowAppPath + "WF/Img/Group.gif";
                formMenu.Update();
            }
        }
        ...
        private string GetEmpOfMenusByEmpNo()
        {
            ...
            QueryObject objMenu = new QueryObject(menus);
            objMenu.AddWhere(MenuAttr.ParentNo, parentNo);
            objMenu.addOrderBy(MenuAttr.Idx);
            objMenu.DoQuery();
            //整理选中项
            foreach (DataRow row in userMenu_dt.Rows)
            {
        ...
        private string GetMenusByParentNo(string parentNo, string checkedMenuIds, string unCheckedMenuIds, bool addChild)
        {
            ...
            //获取菜单
            BP.GPM.Menus menus = new BP.GPM.Menus();
            menus.Retrieve("ParentNo", parentNo, MenuAttr.Idx);
        ...
        public string GetTreeList(Entities ens, string checkIds, string unCheckIds)
        {
            ...
            foreach (EntityTree item in ens)
            {
                ...
                //判断是否还有子节点
                BP.GPM.Menus menus = new BP.GPM.Menus();
                menus.Retrieve("ParentNo", item.No,MenuAttr.Idx);
